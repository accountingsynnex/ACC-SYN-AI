...
function drawerHtml(t){
 ...
 if(ui.drawerTab==='checklist')body=`<div>${t.checklist.map((c,i)=>`<div class="check-item"><input type="checkbox" data-check-index="${i}" ${c.checked?'checked':''} ${canEdit(t)?'':'disabled'}><label>${esc(c.label)} ${c.required?'<span class="required">*</span>':''}</label></div>`).join('')}</div>`;
...
function bindDrawer(t){
 ...
 document.querySelectorAll('[data-check-index]').forEach(c=>c.onchange=()=>{
  const item=t.checklist[+c.dataset.checkIndex];if(!item||!canEdit(t)){c.checked=item?.checked||false;toast('ไม่มีสิทธิ์แก้ไข Checklist ในสถานะนี้');return}
  item.checked=c.checked;t.updatedAt=new Date().toISOString();
  t.activity.push({at:t.updatedAt,action:`${item.checked?'ทำ Checklist เสร็จ':'ยกเลิก Checklist'}: ${item.label}`,by:currentUser().name});
  Store.save();syncTaskProgress(t);
  const drawerBody=document.querySelector('#taskDrawer .drawer-body'),drawerScroll=drawerBody?.scrollTop||0;
 ...
 document.querySelectorAll('[data-transition]').forEach(b=>b.onclick=()=>{const target=b.dataset.transition;if(target==='revision'){openRejectModal(t.id);return}try{TaskService.transition(t.id,target);toast(`ย้ายเป็น ${STATUS[target].label}`);closeDrawer();render({resetScroll:false})}catch(e){toast(e.message)}});
...
function openTaskModal(id=null){
 ...
 modal.querySelector('#taskForm').onsubmit=e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target));d.team=staffLocked?(t?.team||currentUser().team):(d.team||team);d.assignee=staffLocked?(t?.assignee||currentUser().name):(d.assignee||defaultAssignee);d.reviewer=d.reviewer||t?.reviewer||teamMeta(d.team).lead;d.restricted=staffLocked?Boolean(t?.restricted):d.restricted==='true';try{if(t)TaskService.update(t.id,d);else TaskService.create({...d,checklist:['ตรวจสอบเอกสารต้นทาง','แนบ Working file','Self-review'].map((x,i)=>({id:`C${i+1}`,label:x,required:true,checked:false}))});closeModals();closeDrawer();render();toast(t?'บันทึกงานแล้ว':'สร้างงานแล้ว')}catch(err){toast(err.message)}};
 openModal('taskModal');queueCustomSelectRefresh();
}
function openRejectModal(id){const modal=document.getElementById('rejectModal');modal.innerHTML=`<div class="modal-head"><h2>ตีกลับงาน</h2><button class="close-btn" data-close-modal>×</button></div><form id="rejectForm"><div class="modal-body"><div class="field"><label>เหตุผลที่ต้องแก้ไข</label><textarea class="control" name="reason" required placeholder="ระบุสิ่งที่พนักงานต้องแก้ไข"></textarea></div></div><div class="modal-foot"><button type="button" class="btn" data-close-modal>ยกเลิก</button><button class="btn danger">ยืนยันตีกลับ</button></div></form>`;modal.querySelectorAll('[data-close-modal]').forEach(b=>b.onclick=closeModals);modal.querySelector('form').onsubmit=e=>{e.preventDefault();try{TaskService.transition(id,'revision',new FormData(e.target).get('reason'));closeModals();closeDrawer();render({resetScroll:false});toast('ตีกลับงานแล้ว')}catch(err){toast(err.message)}};openModal('rejectModal')}
// ---------------- Dashboard ----------------
