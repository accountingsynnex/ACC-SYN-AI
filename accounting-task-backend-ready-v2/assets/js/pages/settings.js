...
function openGenerateTemplate(id){
 ...
 modal.querySelector('form').onsubmit=e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target));if(state.tasks.some(x=>x.templateId===t.id&&x.period===d.period)){toast('มีงานจาก Annual Task นี้ในงวดดังกล่าวแล้ว');return}try{TaskService.create({title:`${t.name.replace(/^Monthly /,'')} — ${d.period}`,team:t.team,category:t.category,assignee:t.defaultAssignee,reviewer:t.defaultReviewer,priority:t.priority,period:d.period,dueDate:d.dueDate||templateRuleDate(d.period,t.dueRule,5),templateId:t.id,checklist:t.checklist.map((x,i)=>({id:`C${i+1}`,label:x,required:true,checked:false}))});closeModals();renderSettingsModal();toast('สร้างงานจาก Annual Task แล้ว')}catch(err){toast(err.message)}};openModal('templateModal');queueCustomSelectRefresh();
}
