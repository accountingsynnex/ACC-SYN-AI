...
- `pages/settings.js` → **Setting → AI Reference Files** tab (`renderAiReferences`), grouped under
  the same "Configuration" sidebar section as Management and Annual Task. Manages the example
  files AI compares against, one group per task category (same `category` value used as
  `taskType` above). Edit rights use `canManageBeyondStaff()` (`domain/task-service.js` — anyone
  but Staff: reviewer/teamlead/manager, same rule Annual Task management already used); Staff
  gets a read-only list. Hidden entirely with a "not connected" notice if `AiReview.enabled` is
  false. A category with zero reference files means AI skips the check and lets the submission
  through as `pass` (the worker's own behavior, not something this app enforces) — upload at
  least one example per category before relying on the gate.
